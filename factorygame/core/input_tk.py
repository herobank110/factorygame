...
class TkInputHandler(GUIInputHandler):
    ...
    def bind_to_widget(self, in_widget):
        ...
        # Bind mouse events.
        in_widget.bind("<ButtonPress-1>", lambda e: self.register_key_press(
            EKeys.LeftMouseButton, EInputEvent.IE_PRESSED))
        in_widget.bind("<ButtonRelease-1>", lambda e: self.register_key_press(
            EKeys.LeftMouseButton, EInputEvent.IE_RELEASED))

        in_widget.bind("<ButtonPress-2>", lambda e: self.register_key_press(
            EKeys.MiddleMouseButton, EInputEvent.IE_PRESSED))
        in_widget.bind("<ButtonRelease-2>", lambda e: self.register_key_press(
            EKeys.MiddleMouseButton, EInputEvent.IE_RELEASED))

        in_widget.bind("<ButtonPress-3>", lambda e: self.register_key_press(
            EKeys.RightMouseButton, EInputEvent.IE_PRESSED))
        in_widget.bind("<ButtonRelease-3>", lambda e: self.register_key_press(
